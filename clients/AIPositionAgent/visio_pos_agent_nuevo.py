...
class VisionPosDevice:
    def __init__(self, agent: Agent) -> None:
        ...
        # Configuración de la cámara A (Cámara USB 0)
        self.cap_a = cv2.VideoCapture("camara_simulada.mp4") #0
        self.cap_a.set(cv2.CAP_PROP_FPS, 30)
        
        self.cap_b = self.MockBlackCapture(MAX_WIDTH, MAX_HEIGHT)
        ...
        logger.info("VisionPosDevice inicializado con video simulado y cámara B negra.")
        ...
        self.cap_a.set(cv2.CAP_PROP_BRIGHTNESS, 100)
        
        # Configuración de la cámara B (Cámara USB 2)
        ...
        '''
    class MockBlackCapture:
        """Emula un objeto VideoCapture de OpenCV devolviendo siempre frames negros."""
        def __init__(self, width, height):
            self.width = width
            self.height = height

        def read(self):
            # Devuelve True y una matriz de ceros (negro total) con las dimensiones correctas
            frame = np.zeros((self.height, self.width, 3), dtype=np.uint8)
            return True, frame

        def set(self, propId, value):
            pass  # Ignora las configuraciones de FPS/Exposición

        def release(self):
            pass  # No hay hardware que liberar
    ...
    def loop(self):
        try:
            while self.running:
                # 1. Captura de imágenes por hardware
                ret_a, frame_a = self.cap_a.read()
                # Si el video simulado termina, reiniciarlo al fotograma 0 automáticamente
                if not ret_a:
                ...
                # Segmentamos las zonas muy brillantes (Ajusta el 220 según la luz ambiental del laboratorio)
                ...
                for idx, contorno in enumerate(contornos):
                    ...
                    if 100 < area < 5000:
                        ...
                        if M["m00"] != 0:
                            obs_center_x = float(M["m10"] / M["m00"])
                            obs_center_y = float(M["m01"] / M["m00"])
                            
                            # Traducción geométrica a centímetros reales de la arena
                            pt_pixel_obs = np.array([[[obs_center_x, obs_center_y]]], dtype=np.float32)
                            if self.M_trans is not None:
                                pt_real_obs = cv2.perspectiveTransform(pt_pixel_obs, self.M_trans)[0][0]
                                real_obs_x = pt_real_obs[0]
                                real_obs_y = pt_real_obs[1]
                            else:
                                real_obs_x = obs_center_x
                                real_obs_y = obs_center_y
                            
                            # Estimación del radio aproximado del obstáculo en píxeles
                            ...
                            # Pintamos los resultados en la interfaz si está habilitada
                            if self.gui:
                                cv2.drawContours(canvas_red, [contorno], -1, (255, 0, 0), 2)
                                cv2.circle(canvas_red, (int(obs_center_x), int(obs_center_y)), 5, (0, 0, 255), -1)

                # Publicamos la lista completa de obstáculos en el tópico global de la red
...
if __name__ == "__main__":
    ...
    '''agente_integrado = Agent(
        ...
        ip="192.168.10.1",      # IP local de tu Raspberry Pi
        data_port=5559,
        hub_ip="192.168.10.1"   # IP de tu Hub central
    )
    '''
